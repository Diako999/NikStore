...
describe('CartService', () => {
  ...
  let discountsService: jest.Mocked<
    Pick<DiscountsService, 'calculateDiscountedPrice'>
  >;
  ...
  describe('addItem — with system discount', () => {
    ...
    it('applies system discount: price becomes discounted, comparePrice becomes original price', async () => {
      ...
      const expectedPrice =
        RETAIL_PRICE - Math.floor((RETAIL_PRICE * 10) / 100);
  ...
  describe('removeItem', () => {
    ...
    it('throws when item not in cart', async () => {
      redis.get.mockResolvedValue(null);
      await expect(service.removeItem(userId, prodId, varId)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
